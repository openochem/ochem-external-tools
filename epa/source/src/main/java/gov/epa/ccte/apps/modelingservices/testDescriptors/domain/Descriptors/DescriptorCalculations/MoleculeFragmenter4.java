package gov.epa.ccte.apps.modelingservices.testDescriptors.domain.Descriptors.DescriptorCalculations;

import gov.epa.ccte.apps.modelingservices.testDescriptors.domain.Descriptors.DescriptorFactory.DescriptorData;
import gov.epa.ccte.apps.modelingservices.testDescriptors.domain.Descriptors.DescriptorUtilities.AtomicProperties;
...
//		need for IGC50 chemicals like 137-40-6:
		...
		this.getCount("As, any valence","[As]","Any","Any","As","Any","","Any");//TODO change name of frag to "[As]" to avoid name conflict with WHIM "As" descriptor
	...
	private void GetNitroSubstitutionCount(int pos1, int pos2,int pos3) {
		...
		try {
			...
			for (int i=0;i<rs2.getAtomContainerCount();i++) {
				...
				List AtomsWithNitro=this.GetRingAtomsWithNitroGroups(ring,m2);
...
//				System.out.println(count);
	...
	boolean IsNitro(IAtom atom,IAtomContainer molecule) {
		...
		List ca=molecule.getConnectedAtomsList(atom);
