...
import gov.epa.ccte.apps.modelingservices.testDescriptors.domain.Descriptors.DescriptorUtilities.Utilities;
...
public class DescriptorData {
	...
	public Vector<String>getDescriptorValues() {
		...
		try {
			...
			for (int i = 0; i < strFragments.length; i++) {
				...
				if(FragmentList.get(strVar) == null || FragmentList.get(strVar).equals("null")) {
					System.out.println(strVar+ " " + FragmentList.get(strVar));
					values.add("0");
				}
				else
					values.add(myD.format(FragmentList.get(strVar)));
			}
	...
	void loadDefs(String filename) {
		...
		try {
			...
			BufferedReader br=new BufferedReader(new FileReader(EPAServer.getFile(filename)));
	...
	public DescriptorData(boolean also3d) {
		this.htVarDef=new Hashtable();

		if (Load2dDescriptions) this.loadDefs("variable_definitions.txt");
		if (also3d) this.loadDefs("variable_definitions-My3d.txt");
	...
	public static String toStringDescriptorNames(boolean add3d) {

		Vector<String>fieldNames=getDescriptorNames(add3d);
		String result="";
	...
	public String toStringTranspose(boolean add3d) {
		Vector<String>fieldNames=getDescriptorNames(add3d);
		Vector<String>fieldValues=getDescriptorValues();
	...
	public static Vector<String> getDescriptorNames(boolean also3d) {
		...
		try {
			for (int i = 0; i <= varlist.length - 1; i++) {
				...
				for (int j = 0; j < names.length; j++) vecNames.add(names[j]);
			}
